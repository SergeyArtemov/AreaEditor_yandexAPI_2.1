...
(* --- Инструменты, отчеты, настройки ----------------------------------------------------------- *)
  ...
  private
    ...
    procedure RefreshIntervalScheme;
    function FillIntervalScheme : boolean;
    procedure SaveIntervalScheme;
    procedure LoadIntervalScheme;
    procedure EditInterval(aBC : TButtonCategory);
    //function CreateIntervalListOld : boolean;(*FOR DELETE*)
    function CreateIntervalList : boolean;
...
for cnt:=0 to aparent.ControlCount-1
  do if aParent.Controls[cnt] is TSizedPanel
        ...
        with aParent.Controls[cnt],Items[ind] do
          ...
          Pan.Caption:=dop+Trim(Pan.Caption);
          with StdRect do
            begin
          ...
          if not Assigned(Pan.OnDblClick)
             ...
             if Assigned(btnOut) and btnOut.InheritsFrom(TSpeedButton)
                then with (btnOut as TSpeedButton) do
                       begin
                       Left:=-1;
                       Top:=-1;
...
Items[aIndex].Pan.Parent:=Items[aIndex].ParentForm;
Items[aIndex].Pan.Align:=alClient;
Items[aIndex].Pan.CanPanelMove:=false;
Items[aIndex].Pan.CanPanelResize:=false;
...
//with Items[aIndex] do
//  begin
//  Pan.Left  := StdRect.Left;
//  Pan.Top   := StdRect.Top;
//  Pan.Width := StdRect.Right - Pan.Left;
//  Pan.Height:= StdRect.Bottom - Pan.Top;
//  end;
ShowOutBtn(Items[aIndex].Pan, true);
Items[aIndex].Pan.CanPanelMove:=true;
Items[aIndex].Pan.CanPanelResize:=true;
Items[aIndex].Pan.Parent:=Application.MainForm;
...
if spb=SpB_Mode_Delivery
   ...
   if not needDeliveryMode
      then begin
      if SzPanPosList.IsOut(SzPan_Deliverys) then SzPanPosList.SetIn(SzPan_Deliverys);
//      ind:=SzPanPosList.GetIndex(SzPan_Deliverys.Handle);
//      if Assigned(SzPanPosList.Items[ind].ParentForm)
//         then begin
//         SzPanPosList.Items[ind].ParentForm.Close;
//         FreeAndNil(SzPanPosList.Items[ind].ParentForm);
//         end;
      end;
   end
   else
if (spb=SpB_Mode_Map) or
   ...
   if not needMapMode
      then begin
      if SzPanPosList.IsOut(SzPan_Area) then SzPanPosList.SetIn(SzPan_Area);
//      ind:=SzPanPosList.GetIndex(SzPan_Area.Handle);
//      if Assigned(SzPanPosList.Items[ind].ParentForm)
//         then begin
//         SzPanPosList.Items[ind].ParentForm.Close;
//         FreeAndNil(SzPanPosList.Items[ind].ParentForm);
...
if spb=SpB_Mode_Tools
   ...
   needToolsMode:= SzPan_Tools.Visible;
    if not needToolsMode
      then begin
      if SzPanPosList.IsOut(SzPan_Area) then SzPanPosList.SetIn(SzPan_Area);
//      ind:=SzPanPosList.GetIndex(SzPan_Tools.Handle);
//      if Assigned(SzPanPosList.Items[ind].ParentForm)
//         then begin
//         SzPanPosList.Items[ind].ParentForm.Close;
//         FreeAndNil(SzPanPosList.Items[ind].ParentForm);
  ...
  case ARow of
    ...
    begin
      ...
      if(ind < Low(CarList.Items))or(ind > High(CarList.Items))
         then exit;
  System.Move(GoodColors, CurColors, SizeOf(TItemColors)) ;
  with DrGr_CarsList.Canvas do
...
if Sender=BB_DTI_Close
  ...
  if SzPanPosList.IsOut(SzPan_IntervalSchemeEx) then SzPanPosList.SetIn(SzPan_IntervalSchemeEx);
