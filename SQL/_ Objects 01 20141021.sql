...
(SELECT [OrderNo]
  ...
  where convert(date, [Date])>=@Begin and convert(date, [Date])<=@End
...
select  
     [ID]
    ,[acDate]	-- дата события
    ,[acPlace]	-- место события
    ,[acCar]	-- номер автомобиля по Navision (выбирается по дате)
    ,[acDriver]	-- ФИО водителя по Navision (оператору не показывается, выбирается при выборе автомобиля)
    ,[acType]	-- тип (словарь, HardCode (20141015))
    ,[acNote]	-- описание (если тип "Другое" или уточнение)
    ,[clType]	-- тип (словарь, HardCode (20141015))
    ,[clFIO]	-- ФИО звонившего
    ,[clPhone]	-- телефон звонившего
    ,[resNote]	-- заключение по случаю (описание)
	,[resGuilty]-- заключение по случаю (признак виновности)
from dbo.ae_AccidentList(nolock)
...
select 
 ...
 ,t.r.value('@RESGUILTY','[bit]')			as [resGuilty]	-- заключение по случаю (признак виновности)
...
if @Res>0
  ...
  select @verdict=IsNull(SRC.[resNote],'') from @srcTable SRC
  if @verdict='' set @Event = 1 else set @Event = 4
  end
  else
if @Res<0
  begin
...
select @Res      ,@Event , GetDate() , @UserID ,@WS          ,@User,[client_net_address]+':'+cast([client_tcp_port] as varchar)
  FROM [master].[sys].[dm_exec_connections](nolock)
where [session_id]=@@SPID
-->-----------------------------------------
COMMIT TRAN
...
GO
--exec ae_AreaLogList_Load '<REQUEST BEGIN="20141023" END="20141023"/>'
